<div class="container">
    <!-- start: PAGE HEADER -->
    <div class="row">
        <div class="col-sm-12">

            <!-- start: PAGE TITLE & BREADCRUMB -->
            <ol class="breadcrumb">
                <li>
                    <i class="clip-home-3"></i>
                    <a href="#">
                        Home
                    </a>
                </li>
                <li class="active">
                    Publikasi
                </li>
                <li class="active">
                    Event
                </li>
               
            </ol>
            <div class="page-header">
                <h1>Event</h1>
            </div>
            <!-- end: PAGE TITLE & BREADCRUMB -->
        </div>
    </div>
    <!-- end: PAGE HEADER -->
    <!-- start: PAGE CONTENT -->
    <div class="row">
        <div class="col-md-12 space20">
            <a href="?menu=tambah_event" class="btn btn-success add-row pull-right">
                Tambah Event <i class="fa fa-plus"></i>
            </a>
        </div>
        <div class="col-md-12">
            <table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
                <thead>
                    <tr>
                        <th width="10px">No</th>
                        <th style="min-width:200px">Judul</th>
                        <th style="min-width:100px">Tanggal</th>
                        <!-- <th style="min-width:100px">Jam</th> -->
                        <th style="min-width:150px">Lokasi</th>
                        <th style="min-width:100px">Thumbnail</th>
                        <th style="">Jumlah Hit</th>
                        <th style="">Status</th>
                        <th style="min-width:100px">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>1</td>
                        <td>BPSDM Perhubungan Optimalkan Pembelajaran Jarak Jauh</td>
                        <td>13 Juni 2020 12.00 - 15.00</td>
                        <!-- <td></td> -->
                        <td>Poltekbang Palembang</td>
                        <td><img width="100px" src="https://www.koran-jakarta.com/images/article/php7hjl_0_resized.jpg" alt=""></td>
                        <td>1</td>
                        <td><span class="badge badge-success">Publish</span></td>
                        <td>
                        <button class="btn btn-xs btn-info"><i class="fa fa-edit"></i></button>
                            <button class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>
                        </td>
                    </tr>
                  
                </tbody>
            </table>


            <!-- end: DYNAMIC TABLE PANEL -->
        </div>
    </div>
    <!-- end: PAGE CONTENT-->
</div>